...
if (!$enrollment_id) {
    $modal_type = "error";
    $modal_message = "Invalid enrollment ID.";
} else {
    // Get student name for the message
    $stmt = $conn->prepare("SELECT u.name, c.course_name 
                          FROM enrollment e 
    ...
    $student_name = $enrollment ? $enrollment['name'] : 'Student';
    $course_name = $enrollment ? $enrollment['course_name'] : 'course';
    ...
    if ($stmt->execute()) {
        $modal_type = "success";
        $modal_message = "$student_name has been removed from $course_name successfully.";
    } else {
        $modal_type = "error";
        $modal_message = "Error removing enrollment: " . $conn->error;
    }
}
